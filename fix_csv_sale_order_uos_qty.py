...
for sale in sales:
    ...
    if not msg:
        continue
    self.message_post(cr, uid, sale.id, body="<ul>"+msg+"</ul>", subject="Fix CSV imported sale orders (UoS Qty = UoM Qty) to properly compute sale order margin", type='comment', subtype='html')
